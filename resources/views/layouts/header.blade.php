<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Ginecología y Obstetricia</title>
    <link rel="stylesheet" href="{{ asset('css/index.css') }}">

    <!--Iconos-->
    <script src="https://kit.fontawesome.com/eb496ab1a0.js" crossorigin="anonymous"></script>

</head>
<body>
<header class="header">
    <img src="img/nom_logo.png" alt="Logo" style="width: 300px; height: auto;">
    <nav class="nav">
        <ul class="nav-links">
            <li><a href="http://localhost/ginecologia/public/index">Inicio</a></li>
            <li><a href="http://localhost/ginecologia/public/sobre_nosotros">Sobre Nosotros</a></li>
            <li><a class="respli" href="http://localhost/ginecologia/public/contactanos">Contactanos</a></li>
            
        </ul>
    </nav>
    <div class="barrs">
        <div class="line"></div>
        <div class="line"></div>
        <div class="line"></div>
    </div>
    <a class="btn" href="http://localhost/ginecologia/public/contactanos"><div class="button">Contáctanos</div></a>
    <a class="btn" href="http://localhost/ginecologia/public/login"><div class="button">Inicie Sesión</div></a>
</header>
